...
** How to Setup gem5
   ...
   Here, you can either:
   - Follow the steps on the gem5 website to get the latest version of it and
     use the new recommended methods (e.g., Docker container). You may have to
     adapt our work to this new version, and the results may not be the same.
   - Reproduce exactly what we did, with a bit of tinkering.

   First, install the recommendedg packages:
   ...
   #+END_SRC
    ...
    If everything is working, we should be able to display the help of one
    simulation script:
    ...
    Let's create the images we need to simulate Spectre on a full-system
    simulation:
